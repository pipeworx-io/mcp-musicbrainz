...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'MusicBrainz');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'search_artists',
    description: 'Search for music artists by name. Returns artist IDs, names, types, and countries. Use get_artist to fetch full discography and biographical details.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  },
  {
    ...
    description: 'Search for albums and releases by title or artist name. Returns release IDs, titles, artists, release dates, and formats.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
...
async function searchArtists(query: string, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/artist?${params.toString()}`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'MusicBrainz error');
...
async function getArtist(id: string) {
  const res = await pwFetch(`${BASE_URL}/artist/${encodeURIComponent(id)}?inc=releases&fmt=json`, {
    headers: HEADERS,
  });
  if (!res.ok) throw await httpError(res, 'MusicBrainz error');
...
async function searchReleases(query: string, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/release?${params.toString()}`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'MusicBrainz error');
