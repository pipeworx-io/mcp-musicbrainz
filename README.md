...
`tools/list` at `https://gateway.pipeworx.io/musicbrainz/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_artists \
  -H 'Content-Type: application/json' \
  -d '{"query":"The Beatles"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_artists`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "musicbrainz": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-musicbrainz"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-musicbrainz
```
